function Mesh() {
    ...
    this.programLoaded = function(program) {
        ...
        // program.vertexTextureCoordsAttribute = gl.getAttribLocation(program, 'aVertexTextureCoord');
    ...
    this.init = function(jsonstring) {
        ...
        var that = this;
        function programLoaded(prog) {
            that.programLoaded(prog);
        }

        for (var m in mesh.materials) {
            var material = mesh.materials[m];
            var prog = loadProgram(material.vertexshader, material.fragmentshader, programLoaded);
    ...
    this.draw = function() {
        ...
        for (var p in this.programs) {
            ...
            // if (program.vertexTextureCoords) {
            //     gl.enableVertexAttribArray(program.vertexTextureCoordsAttribute);
            //     gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexTextureCoordsBuffer);
            //     gl.vertexAttribPointer(program.vertexTextureCoordsAttribute, 2, gl.FLOAT, false, 0, 0);
            // }
            ...
            gl.drawElements(gl.TRIANGLES, program.numindices, gl.UNSIGNED_SHORT, start * 2); // represent start in bytes
